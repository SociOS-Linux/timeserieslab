...
EXPECTED_SERVICE_ID = "timeserieslab.holmes.timeseries-pilot"
EXPECTED_FUNCTION = "timeseries"
...
def main() -> int:
    ...
    if manifest.get("function") != EXPECTED_FUNCTION:
        return fail("manifest function must be timeseries")
    service = manifest.get("service", {})
    if service.get("id") != EXPECTED_SERVICE_ID:
        return fail("manifest service.id mismatch")
    if service.get("ownerRepository") != "SociOS-Linux/timeserieslab":
    ...
    print("OK: timeserieslab manifest and fixtures validated")
    return 0
